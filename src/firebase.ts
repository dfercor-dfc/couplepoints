...
export async function createRoom(roomId: string, initialState: RoomState) {
  await set(roomRef(roomId), initialState)
}
export async function getRoomOnce(roomId: string): Promise<RoomState | null> {
...
export async function updateRoom(roomId: string, partial: Partial<RoomState>) {
  await update(roomRef(roomId), partial)
}

export interface Player {
  id: number; name: string; emoji: string; points: number
  streak: number; totalEarned: number; totalSpent: number
}
export interface PendingTask {
  id: string; taskName: string; taskIcon: string; pts: number
  requestedBy: number; pendingFor: number; timestamp: number
